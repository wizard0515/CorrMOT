...
class MotLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        opt = self.opt
        hm_loss, wh_loss, off_loss, id_loss, corr_loss= 0, 0, 0, 0, 0
        for s in range(opt.num_stacks):
            ...
            if opt.wh_weight > 0:
                ...
                corr_loss += self.crit_reg(
                    output['wh'], batch['reg_mask'],
                    batch['ind'], batch['wh']) / opt.num_stacks/10
        ...
        det_loss = opt.hm_weight * hm_loss + opt.wh_weight * wh_loss + opt.off_weight * off_loss + opt.wh_weight * corr_loss
        ...
        loss_stats = {'loss': loss, 'hm_loss': hm_loss,
                      'wh_loss': wh_loss, 'off_loss': off_loss, 'id_loss': id_loss, 'corr_loss':corr_loss}
...
class MotTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'wh_loss', 'off_loss', 'id_loss', 'corr_loss']
        loss = MotLoss(opt)
